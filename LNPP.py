...
def gram_schmidt_orthonormalize(vectors):
    """
    对一组特征向量进行格拉姆-施密特正交化和归一化。

    参数:
    vectors -- 输入的特征向量矩阵，每一列是一个特征向量。
    ...
    for i in range(n):
        ...
        # 减去在之前向量上的投影
        for j in range(i):
            proj = np.dot(Q[:, j], v) * Q[:, j] / np.dot(Q[:, j], Q[:, j])
            v = v - proj
...
def lnpp(A, epsilon, k):
    ...
    epsilon_proportion_for_eigenvalues = 0.02  # 假设特征值分配 2% 的隐私预算
    epsilon_values = np.full(k , epsilon * (1-epsilon_proportion_for_eigenvalues) / (k ))  # 简单等分 epsilon
    epsilon_0 = epsilon * epsilon_proportion_for_eigenvalues  # ε0 用于特征值
    epsilon_k = epsilon_values[0:]  # ε1,...,εk 用于特征向量
    ...
    noisy_eigenvectors = gram_schmidt_orthonormalize(noisy_eigenvectors)
